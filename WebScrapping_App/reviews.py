...
class GetReviews:
    ...
    def navigatetoApp(self,searchString):
        try:
            ...
            resp=urlopen(req)
            logger.app_logger().log(log_file,'opened application url '+app_url)
            global reviewpage
            reviewpage= resp.read()
            if "ratingvalue" in str(reviewpage):
                global productname
                productname=searchString.replace("-"," ")
                logger.app_logger().log(log_file,'navigated to the application url '+app_url+' Able to find the product')
    ...
    def get_reviews_from_ui(self):
        try:
            ...
            logger.app_logger().log(log_file,'All rating fields are '+rating_dict)
